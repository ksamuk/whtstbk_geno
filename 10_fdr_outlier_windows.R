...
sel_df <- left_join(out_df %>% select(chr, pos1, matches("adj")), stat_df %>% select(-pos2))
...
snp_long %>% 
  ...
  filter(grepl("rsb_wht_cmn|fst_wht_cmn|fst_cbr_cmn|tajd_wht|tajd_cmn|pi_wht|pi_cmn", stat)) %>%
